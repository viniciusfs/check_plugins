...
def main():
    ...
    if memory_usage['perc_inuse'] <= warning:
        print 'Memory %s %.2f%% | %s' % ('OK', memory_usage['perc_inuse'], print_perfdata(memory_usage))
        exit(OK)

    if memory_usage['perc_inuse'] > warning and memory_usage['perc_inuse'] < critical:
        print 'Memory %s %.2f%% | %s' % ('WARNING', memory_usage['perc_inuse'], print_perfdata(memory_usage))
        exit(WARNING)

    if memory_usage['perc_inuse'] >= critical:
        print 'Memory %s %.2f%% | %s' % ('CRITICAL', memory_usage['perc_inuse'], print_perfdata(memory_usage))
        exit(CRITICAL)
